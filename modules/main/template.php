<?php
echo "Основная страница";
?>
<main>
    <div class="wraper">
        <h1>НОВЫЕ ПООСТУПЛЕНИЯ ВЕСНЫ</h1>
        <div class="wraper__slogan">Мы подготовили для Вас лучшие новинки сезона</div>
        <div class="wraper__look-new">
            <div class="wraper__look-new__item look-new__item">
                Посмотреть новинки
            </div>
        </div>

        <div class="wraper__gooods-block">
            <div class="wraper__gooods-block__block col-sm-3">
                <div class="wraper__gooods-block__block__2block jins"></div>
                <div class="wraper__gooods-block__block__1block belt"></div>
            </div>

            <div class="wraper__gooods-block__block col-sm-3">
                <div class="wraper__gooods-block__block__1block">
                    <div class="block__quests-container">
                        <img class="quests" src="img/icons/attention-sign-outline.png" />
                    </div>
                    <div class="block__text">каждый сезон мы подготавливаем для Вас исключительно лучшую модну одежду. Следит за нашими новинками</div>
                </div>
                <div class="wraper__gooods-block__block__1block jins">
                    <div class="block__content">
                        <h3>ДЖИНСЫ</h3>
                        <div>от 3200.</div>
                    </div>
                </div>
                <div class="wraper__gooods-block__block__1block dark">
                    <div class="block__content">
                        <h3>АКСЕСУАРЫ</h3>
                        <div>от 3200.</div>
                    </div>
                </div>
            </div>

            <div class="wraper__gooods-block__block col-sm-3">
                <div class="wraper__gooods-block__block__1block boots"></div>
                <div class="wraper__gooods-block__block__1block">
                    <div class="block__quests-container">
                        <img class="quests" src="img/icons/attention-sign-outline.png" />
                    </div>
                    <div class="block__text">Самые низкие цены в Москве. Нашли дешевле? Вернем разницу.</div>
                </div>
                <div class="wraper__gooods-block__block__1block runer">
                    <div class="runer__overlay"></div>
                </div>
            </div>

            <div class="wraper__gooods-block__block col-sm-3">
                <div class="wraper__gooods-block__block__1block dark">
                    <div class="block__content">
                        <h3>ЭЛЕГАНТНАЯ ОБУВЬ</h3>
                        <div>БОТИНКИ, КРОССОВКИ</div>
                    </div>
                </div>
                <div class="wraper__gooods-block__block__2block clothes"></div>
            </div>
        </div>

        <div class="wraper__bottom-info">
            <div class="bottom-info__title">БУДЬ ВСЕГДА В КУРСЕ ВЫГОДНЫХ ПРЕДЛОЖЕНИЙ</div>
            <div class="bottom-info__subtext">подписывайся и следи за новинками и выгодными предложениями.</div>

            <form id="mail_send">
                <input name="mail" type="text" placeholder="email" />
                <input type="submit" value="Записаться" />
            </form>
            <span id="system-message"></span>
        </div>
    </div>
</main>